...
#   define ALL_CELL_MASK    0x003F
// if the mr-bms771 board with the MC33771C chip
#else
#   define ALL_CELL_MASK    0x3FFF
#endif
    ...
    for(i = 0; i < 7; i++)
    {
        ...
        if(ANxbits & (1<<i))
        {
            ...
            if(lowerTH != NULL)
            {
                ...
                lvRetValue |= bcc_spiwrapper_BCC_Reg_Write(drvConfig, cid, (BCC_REG_TH_AN0_UT_ADDR - i), regVal, NULL);
            ...
            if(upperTH != NULL)
            {
                ...
                // write the lower temperature threshold
                lvRetValue |= bcc_spiwrapper_BCC_Reg_Write(drvConfig, cid, (BCC_REG_TH_AN0_OT_ADDR - i), regVal, NULL);
 ...
 */
    ...
    uint8_t sleepCurrentmA)
 {
    ...
    // get the registervalue to later compare
    ...
    lvRetValue = bcc_spiwrapper_BCC_Reg_Write(drvConfig, cid, BCC_REG_TH_ISENSE_OC_ADDR, regVal, NULL);
 ...
 */
    ...
    uint8_t newCellCount)
 {
    ...
    if(newCellCount < N_CELLS_MIN || newCellCount > N_CELLS_MAX)
    {
    ...
    {
        // check if it is the first connected cells
        if(i >= FIRST_BCC_CELLS)
        {
            // calculate the BCC pin index
            regVal |= 1 << ((N_CELLS_MAX-newCellCount) + i);
        }
        else
        {
            // it is the first x cells
            regVal |= 1 << i;
        }
    }
    ...
    // check if worked
    if(regVal != (retRegVal & ALL_CELL_MASK))
    {
        // report to user
        cli_printfError("bcc_configuration ERROR: update th en failed! %d != %d\n",
            regVal, (retRegVal & ALL_CELL_MASK));
    }
