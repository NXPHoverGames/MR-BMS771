...
// with charging, the BMS772 does measure own board current that does not go into the battery, substract this.
#if BOARD_NAME == RDDRONE_BMS772_BOARD
    ...
    error = bcc_spiwrapper_BCC_Reg_Read(drvConfig, BCC_CID_DEV1, BCC_REG_MEAS_CELLX_ADDR_MC3377X_START,
        (BCC_REG_MEAS_ANX_ADDR_END - BCC_REG_MEAS_CELLX_ADDR_MC3377X_START) + 1,
        (uint16_t*)(measurements + ((uint8_t)BCC_MSR_CELL_VOLTX)));
    ...
    for(i = 0; i < pCommonBatteryVariables->N_cells; i++)
    {
        ...
        // check if it is not the first connected cells
        if(i >= FIRST_BCC_CELLS)
        {
            // calculate the BCC pin index
            variable1.uint8Var = (N_CELLS_MAX - pCommonBatteryVariables->N_cells) + i;
        }
        else
        {
    ...
    *lowestCellVoltageAdr = lowestCellVoltage;
    ...
    if(pCommonBatteryVariables->sensor_enable & 1)
    {
        ...
        error = getNtcCelsius(measurements[BCC_MSR_AN0-ANX_C_BATT], &(variable1.int16Var));
    ...
    error = getNtcCelsius(measurements[BCC_MSR_AN0-ANX_C_AFE], &(variable1.int16Var));
    ...
    error = getNtcCelsius(measurements[BCC_MSR_AN0-ANX_C_T], &(variable1.int16Var));
    ...
    error = getNtcCelsius(measurements[BCC_MSR_AN0-ANX_C_R], &(variable1.int16Var));
...
bcc_status_t bcc_monitoring_checkNCells(bcc_drv_config_t* const drvConfig, bool* nCellsOk)
{
    ...
    *nCellsOk = 0;
    ...
    for(i = 0; i < nCells; i++)
    {
        // check if it is the first cells
        if(i >= FIRST_BCC_CELLS)
        {
            // calculate the BCC pin index
            bccIndex = (N_CELLS_MAX - nCells) + i;
        }
        else
        {
            // it is the first connected cells
        ...
        ((cellVoltageTotal - stackVoltage) < -(STACK_VOLTAGE_DIFFERENCE_ERROR * nCells)))
    {
        ...
        *nCellsOk = 0;
        ...
        // limit upper
        if(nCellsProb > N_CELLS_MAX)
        {
            // N_CELLS_MAX is the max
            nCellsProb = N_CELLS_MAX;
        }
        // limit lower
        else if(nCellsProb < N_CELLS_MIN)
        {
            // N_CELLS_MIN is the min
            nCellsProb = N_CELLS_MIN;
        }
...
/*
 * @brief   This function is used to get the battery (stack) voltage
 *          it will read the battery voltage without reading the other measurements
 * @note    A measurement should be done first
 * @warning It will get the stack voltage, not add all cell voltage measurements
 *          This is different how normally the V_BATT is calculated
 *
 * @param   drvConfig the address the BCC driver configuration
 *
 * @return  bcc_status_t Error code
 */
bcc_status_t bcc_monitoring_getStackVoltage(bcc_drv_config_t* const drvConfig)
{
    bcc_status_t lvRetValue = BCC_STATUS_PARAM_RANGE;
    uint16_t     regVal;
    float        stackVoltage;

    // read the battery voltage (AN4)
    lvRetValue =
        bcc_spiwrapper_BCC_Reg_Read(drvConfig, BCC_CID_DEV1, BCC_REG_MEAS_STACK_ADDR, 1, &regVal);

    // check for errors
    if(lvRetValue != BCC_STATUS_SUCCESS)
    {
        cli_printfError("bcc_monitoring_getStackVoltage ERROR: reading stack voltage failed: %d\n", lvRetValue);
        return lvRetValue;
    }
    ...
    if(data_setParameter(V_BATT, &stackVoltage))
    {
        cli_printfError("bcc_monitoring_getStackVoltage ERROR: setting new data went in the update wrong! par %d: %.3f \n",
            V_BATT, stackVoltage);
...
bcc_status_t bcc_monitoring_getCellVoltages(bcc_drv_config_t* const drvConfig)
{
    ...
    lvRetValue =
        bcc_spiwrapper_BCC_Reg_Read(drvConfig, BCC_CID_DEV1, (BCC_REG_MEAS_CELLX_ADDR_END - (N_CELLS_MAX - 1)),
            N_CELLS_MAX, regVal);
    ...
    for(i = 0; i < nCells; i++)
    {
        // check if it isn't is one of the first cells
        if(i >= FIRST_BCC_CELLS)
        {
            // calculate the BCC pin index
            bccIndex = (N_CELLS_MAX - nCells) + i;
        ...
        {
            // it is one of the first connected cells
            bccIndex = i;
        }

        // Mask the registers
        regVal[(N_CELLS_MAX - 1) - bccIndex] &= BCC_R_MEAS_MASK;

        // convert the cell voltage to a float
        cellVoltage = BCC_GET_VOLT(regVal[(N_CELLS_MAX - 1) - bccIndex]) / UV_TO_V;
...
bcc_status_t bcc_monitoring_getIsenseOpenLoad(bcc_drv_config_t* const drvConfig, bool* openLoadDetected)
{
    ...
    lvRetValue =
        bcc_spiwrapper_BCC_Reg_Write(drvConfig, BCC_CID_DEV1, BCC_REG_SYS_CFG1_ADDR, regVal[0], NULL);
    if(lvRetValue != BCC_STATUS_SUCCESS)
    {
        // unlock the BCC SPI until done
        if(spi_lockNotUnlockBCCSpi(false))
    ...
    lvRetValue =
        bcc_spiwrapper_BCC_Reg_Write(drvConfig, BCC_CID_DEV1, BCC_REG_SYS_DIAG_ADDR, regVal[0], NULL);
    if(lvRetValue != BCC_STATUS_SUCCESS)
    {
        // unlock the BCC SPI until done
        if(spi_lockNotUnlockBCCSpi(false))
...
int bcc_monitoring_calibrateSoC(bool calibrateARem, bool currentCheck)
{
    ...
    if(currentCheck)
    {
        ...
        if(data_getParameter(I_SLEEP_OC_DISCHARGE, &nCells, NULL) == NULL)
        {
            cli_printfError("bcc_monitoring_calibrateSoC ERROR: getting sleep current discharge went wrong!\n");
        }
        // if it went good
        else
        {
