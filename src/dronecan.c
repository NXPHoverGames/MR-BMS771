...
#   define APP_NODE_NAME            "com.nxp.mr.bms771"
...
#else
#   error add this boards name to APP_NODE_NAME
#endif
...
int dronecan_initialize(void)
{
    ...
    if(!gDronecanInitialized)
    {
        ...
        {
            // output to user
            cli_printfError("dronecan_initialize ERROR: failed to initialize blinker sem! error: %d\n", ret);
        }
        else
        {
            // initialize the semaphores
            ...
            {
                // output to user
                cli_printfError("dronecan_initialize ERROR: failed to initialize wait sem! error: %d\n", ret);
            }
            else
            {
                // create the task
                ...
                {
                    int errcode = errno;
                    cli_printfError("dronecan_initialize ERROR: Failed to start CAN LED task: %d\n", errcode);
                }
                else
                {
                    // it is now running
...
static int32_t dronecan_task_initialize(DroneCanardInstance *ins, CanardSocketInstance *sock_ins)
{
    ...
    // turn on LED
    if(dronecan_setLed(true))
    {
        cli_printfError("DroneCAN ERROR: can't turn on LED!\r\n");
    }
#endif
...
int dronecan_setLed(bool LedOn)
{
    ...
    {
        cli_printfError("dronecan_setLed ERROR: not initialized!\r\n");
        return -1;
    }
...
static int canLedBlinkTaskFunc(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        if(gStopCANLed)
        {
            ...
            {
                cli_printfError("canLedBlinkTaskFunc ERROR: can't toggle CAN_LED");
            }
        ...
        if(ret)
        {
            cli_printfError("canLedBlinkTaskFunc ERROR: failed to get time! errno: %d\n", errno);
        }
        ...
        if(gCanStateForLED == CAN_STATE_RUNNING)
        {
            ...
            {
                onOffTimems = DRONECAN_RUNNING_LED_OFF_TIME_MS;
            }
            else
            {
                onOffTimems = DRONECAN_RUNNING_LED_ON_TIME_MS;
            }
        }
        ...
        else if (gCanStateForLED == CAN_STATE_READY)
        {
            ...
            if(canBlinkLedOff == CAN_LED_OFF)
            {
                onOffTimems = DRONECAN_READY_LED_OFF_TIME_MS;
            }
            else
            {
                onOffTimems = DRONECAN_READY_LED_ON_TIME_MS;
            }
        }
