...
/*! @brief  semaphore for the LED toggle wait */
...
int ledState_initialize(LEDColor_t startColor, bool skipSelfTest)
{
    ...
    {
        // output to user
        cli_printfError("ledState ERROR: failed to initialize blinker sem! error: %d\n", lvRetValue);
        return lvRetValue;
    }
    ...
    {
        // output to user
        cli_printfError("ledState ERROR: failed to initialize wait sem! error: %d\n", lvRetValue);
        return lvRetValue;
    }
