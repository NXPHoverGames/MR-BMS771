...
#   define NORMAL_CURRENT_FACTOR_DC_DC 700 // needed for converting DC/DC input voltage to input current
#endif
...
#define CURRENT_ROLLING_AVG_SIZE 5 //!< The size of the rolling average of the current [3ms / meas]
...
typedef enum
{
    ...
    BMS_SW_CELL7_OV = (1 << BMS_FAULT_CELL7_BIT_SHIFT), /*!< there is a SW OV fault with cell 7 */
    BMS_SW_CELL8_OV = (1 << BMS_FAULT_CELL8_BIT_SHIFT), /*!< there is a SW OV fault with cell 8 */
    BMS_SW_CELL9_OV = (1 << BMS_FAULT_CELL9_BIT_SHIFT), /*!< there is a SW OV fault with cell 9 */
    BMS_SW_CELL10_OV = (1 << BMS_FAULT_CELL10_BIT_SHIFT), /*!< there is a SW OV fault with cell 10 */
    BMS_SW_CELL11_OV = (1 << BMS_FAULT_CELL11_BIT_SHIFT), /*!< there is a SW OV fault with cell 11 */
    BMS_SW_CELL12_OV = (1 << BMS_FAULT_CELL12_BIT_SHIFT), /*!< there is a SW OV fault with cell 12 */
    BMS_SW_CELL13_OV = (1 << BMS_FAULT_CELL13_BIT_SHIFT), /*!< there is a SW OV fault with cell 13 */
    BMS_SW_CELL14_OV = (1 << BMS_FAULT_CELL14_BIT_SHIFT), /*!< there is a SW OV fault with cell 14 */
    ...
    newMeasurementsCallbackFunction p_newMeasurementsCallbackFunction, bool skipSelfTest)
{
    ...
    skipSelfTest = true;
    cli_printfWarning("Warning skipping gate and current sense self test\n");
    ...
    if(!skipSelfTest)
    {
        ...
        if(data_getParameter(I_SLEEP_OC_DISCHARGE, &sleepCurrentmA, NULL) == NULL)
        {
            cli_printfError("batManagement ERROR: getting sleepcurrent went wrong!\n");
            sleepCurrentmA = I_SLEEP_OC_DISCHARGE_DEFAULT;
        }
...
/*!
 * @brief   This function is used to set the 12V external supply on or off.
 * @note    Only works for the MR-BMS771, will return 0 for other boards
 *
 * @param   on if true the VCC_12V_EXT will be set on, otherwise it will be set off
 *
 * @return  If successful, the function will return zero (OK). Otherwise, an error number will be returned to
 *          indicate the error.
 */
int batManagement_set12VExtOn(bool on)
{
#if BOARD_NAME == MR_BMS771_BOARD
    // Set the GPIO and check the state
    return gpio_writePin(EXT_12V_ON, on);
#else
    return 0;
#endif
}
...
int batManagement_checkFault(uint32_t *BMSFault, bool resetFaultPin)
{
    ...
    *BMSFault = 0;
        ...
        *BMSFault |= gSWFaultVariable;
        ...
        if(gSWFaultVariable &
            (BMS_SW_CELL1_OV + BMS_SW_CELL2_OV + BMS_SW_CELL3_OV + BMS_SW_CELL4_OV + BMS_SW_CELL5_OV +
                BMS_SW_CELL6_OV + BMS_SW_CELL7_OV + BMS_SW_CELL8_OV + BMS_SW_CELL9_OV + BMS_SW_CELL10_OV +
                BMS_SW_CELL11_OV + BMS_SW_CELL12_OV + BMS_SW_CELL13_OV + BMS_SW_CELL14_OV))
        {
    ...
    {
        // if there is a bit set, output the full register, for the registers, see bcc_fault_status_t in bcc.h
    ...
    for(i = 0; i < NumCells; i++)
    {
        ...
        if(i >= FIRST_BCC_CELLS)
        {
            // calculat the BCC pin index
            checkBCCCell = (N_CELLS_MAX - NumCells) + i;
        }
        else
        {
            // it is the first connected cells
            checkBCCCell = i;
        }
...
#ifdef DEBUG_FAULT_STATUS
        cli_printf("CSB wakeup detected!\n");
#endif
    }
    ...
    switch(mode)
    {
        case AFE_NORMAL:
            ...
            do
            {
                ...
                lvRetValue = BCC_wakeUpBCC(&gBccDrvConfig, BCC_CID_DEV1);

                if(lvRetValue != BCC_STATUS_SUCCESS && lvRetValue != BCC_STATUS_NULL_RESP &&
                    lvRetValue != BCC_STATUS_CRC)
                {
                    cli_printfError("batManagement_setAFEMode(AFE_NORMAL) ERROR: failed to send wakeup frame to BCC: %d\n",
                        lvRetValue);
                }
        ...
        case AFE_SLOW:
            ...
            do
            {
                ...
                if(lvRetValue != BCC_STATUS_SUCCESS && lvRetValue != BCC_STATUS_NULL_RESP &&
                    lvRetValue != BCC_STATUS_CRC)
                {
                    cli_printfError("batManagement_setAFEMode(AFE_SLOW) ERROR: failed to send wakeup frame to BCC: %d\n",
                        lvRetValue);
                }
            ...
            if(BCC_VerifyCom(&gBccDrvConfig, BCC_CID_DEV1))
            {
                ...
                do
                {
                    ...
                    if(lvRetValue != BCC_STATUS_SUCCESS && lvRetValue != BCC_STATUS_NULL_RESP &&
                        lvRetValue != BCC_STATUS_CRC)
                    {
                        cli_printfError("batManagement_setAFEMode(AFE_SLEEP_MEAS) ERROR: failed to send wakeup frame to BCC: %d\n",
                            lvRetValue);
                    }
            ...
            if(previousAFEMode != AFE_NORMAL)
            {
                ...
                do
                {
                    ...
                    if(lvRetValue != BCC_STATUS_SUCCESS && lvRetValue != BCC_STATUS_NULL_RESP &&
                        lvRetValue != BCC_STATUS_CRC)
                    {
                        cli_printfError("batManagement_setAFEMode(AFE_SLEEP) ERROR: failed to send wakeup frame to BCC: %d\n",
                            lvRetValue);
                    }
...
int batManagement_changedParameter(parameterKind_t changedParam, void *newValue, void *extraValue)
{
    ...
    switch(changedParam)
    {
        ...
        case V_MAX:
            ...
            variable1.uint16Var = (uint16_t)((((int)(uint8_t *)newValue) * 1000) / *(float *)extraValue);

            // seting PCB voltage threshold registers
            lvRetValue = bcc_configuration_changeANxVTH(&gBccDrvConfig, BCC_CID_DEV1, (1 << ANX_V_OUT),
                NULL, &variable1.uint16Var);

            // check for error
            if(lvRetValue != BCC_STATUS_SUCCESS)
            {
                cli_printfError("batManagement_changedParameter ERROR: failed to set PCB OV BCC: %d\n", lvRetValue);
                return lvRetValue;
            }
            break;
        // in case the output voltage factor changes
        case F_V_OUT_DIVIDER_FACTOR:
 ...
 *          will trigger the main to react on it.
 *          Afterwards it will check balancing and trigger to update measurements.
 *          Every MEASURE_CURRENT_US the current is measured and every t-meas
...
static int batManagement_batManagTaskFunc(int argc, char *argv[])
{
    ...
    float   currentArr[CURRENT_ROLLING_AVG_SIZE] = {0};
    uint8_t currentArrIndex                      = 0;
    ...
    while(1)
    {
        ...
        else if(bcc_status == BCC_STATUS_SUCCESS)
        {
            ...
            // substract the old current from the sum and add the new
            sumOfCurrent -= currentArr[currentArrIndex];
            sumOfCurrent += commonBatteryVariables.I_batt;

            // add the new current to the rolling average
            currentArr[currentArrIndex] = commonBatteryVariables.I_batt;
            currentArrIndex = (currentArrIndex + 1) % CURRENT_ROLLING_AVG_SIZE;

            // get the average current (rolling average of CURRENT_ROLLING_AVG_SIZE))
            currentAvg = sumOfCurrent / CURRENT_ROLLING_AVG_SIZE;

            // make sure the main state checks transitions based on the new average current
            if(g_checkForTransitionCurrentCallbackFunctionfp(&(currentAvg)))
            {
...
static int checkAllMeasurements(commonBatteryVariables_t *pCommonBatteryVariables)
{
    ...
    else if(variable1.uint8Var)
    {
        ...
        // get the sleep overcurrent for charging
        if(data_getParameter(I_SLEEP_OC_CHARGE, &(variable1.uint8Var), NULL) == NULL)
        {
            cli_printfError("checkAllMeasurements ERROR: getting sleep overcurrent charge went wrong!\n");
        ...
        // if the 1s avg current minus the sleep overcurrent charge is more than 0
...
static bcc_status_t BatManagement_initializeBCC(void)
{
    ...
    gBccDrvConfig.device[BCC_FIRST_INDEX]  = BCC_DEVICE;
    ...
    cli_printf("Will do SW reset!!\n");

    lvRetValue = BCC_wakeUpBCC(&gBccDrvConfig, BCC_CID_DEV1);

    if(lvRetValue != BCC_STATUS_SUCCESS && lvRetValue != BCC_STATUS_NULL_RESP &&
        lvRetValue != BCC_STATUS_COM_RC)
    {
        cli_printfError("BatManagement_initializeBCC ERROR: failed to send wakeup frame to BCC: %d\n",
            lvRetValue);
    }
    ...
    lvRetValue = BCC_SoftwareReset(&gBccDrvConfig, BCC_CID_DEV1);

    if(lvRetValue != BCC_STATUS_SUCCESS && lvRetValue != BCC_STATUS_NULL_RESP)
    {
        cli_printfError("Error BCC_SoftwareReset went wrong: %d\n", lvRetValue);
        return lvRetValue;
    }
    ...
    //status = BCC_AssignCid(gBccDrvConfig, BCC_CID_DEV1);
    ...
    // if the temp sensor is on
    if(nCells)
    {
        configBits = ~(uint8_t)((1 << ANX_C_R) | (1 << ANX_C_AFE) | (1 << ANX_C_T) | (1 << ANX_C_BATT));
    }
    else
    {
        configBits = ~(uint8_t)((1 << ANX_C_R) | (1 << ANX_C_AFE) | (1 << ANX_C_T));
    }

    // Disable the non-temp analog inputs
    lvRetValue = bcc_configuration_disableNEnableANx(&gBccDrvConfig, BCC_CID_DEV1, configBits, true);  // check for error

    if(lvRetValue != BCC_STATUS_SUCCESS)
    {
        cli_printfError("BatManagement_initializeBCC ERROR: failed to disable other ANx: %d\n",
            lvRetValue);
        return lvRetValue;
    }
#endif
    ...
    if(data_getParameter(I_SLEEP_OC_WAKEUP, &sleepCurrent, NULL) == NULL)
    {
        cli_printfError("BatManagement_initializeBCC ERROR: failed to get sleep current\n");
        sleepCurrent = I_SLEEP_OC_WAKEUP_DEFAULT;
    }
