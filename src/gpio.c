 ...
 */
static uint32_t gInterruptPinsISR = 0;
...
int gpio_init(bool skipSelfTest)
{
    ...
    // write the CAN1 LED pin ON
    retValue = gpio_writePin(LED_CAN1, CAN_LED_ON);

    // check if it went wrong
    if(retValue)
    {
        cli_printfError("GPIO ERROR: writing LED_CAN1 CAN_LED_ON went wrong!\n");
        cli_printf("SELF-TEST GPIO: \e[31mFAIL\e[39m\n");
        return retValue;
    }

    // write the CAN1 enable pin ON
    retValue = gpio_writePin(CAN1_EN, 0);

    // check if it went wrong
    if(retValue)
    {
        cli_printfError("GPIO ERROR: writing CAN1_EN 0 went wrong!\n");
        cli_printf("SELF-TEST GPIO: \e[31mFAIL\e[39m\n");
        return retValue;
    }
#endif
...
#ifdef END_NON_IMPLEMENTED_GPIOS
    if(pin >= START_NON_IMPLEMENTED_GPIOS && pin <= END_NON_IMPLEMENTED_GPIOS)
    {
        //cli_printf("gpio_readPin: non implemented pin%d\n", pin);
        return 0;
    }
#endif
...
int gpio_writePin(pinEnum_t pin, bool newValue)
{
    ...
    if(pin >= START_NON_IMPLEMENTED_GPIOS && pin <= END_NON_IMPLEMENTED_GPIOS)
    {
        //cli_printf("gpio_writePin: non implemented pin%d\n", pin);
        return 0;
    }
#endif
...
int gpio_registerISR(uint32_t IsrPins, _sa_sigaction_t pinISRHandler)
{
    ...
    if(IsrPins == 0)
    {
        cli_printfError("GPIO ERROR: No pins assigned to have an ISR see IsrPins!\r\n");
    }
...
#ifdef END_NON_IMPLEMENTED_GPIOS
        if(i >= START_NON_IMPLEMENTED_GPIOS && i <= END_NON_IMPLEMENTED_GPIOS)
        {
            cli_printf("gpio_registerISR: non implemented pin%d\n", i);
            return 0;
        }
        else
        {
