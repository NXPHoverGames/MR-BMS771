 ...
 ****************************************************************************/
#if BOARD_NAME == RDDRONE_BMS772_BOARD
#   define BMS_VERSION_STRING_ADDITION "11.0-RDDRONE-BMS772\n"
#elif BOARD_NAME == MR_BMS771_BOARD
#   define BMS_VERSION_STRING_ADDITION "11.0-MR-BMS771\n"
#endif
//#define DONT_DO_CAN
...
//! @brief defined number to indicate CAN is OFF
#define CAN_OFF_NUM     0
//! @brief defined number to indicate DroneCAN is used
#define DRONECAN_NUM    1
//! @brief defined number to indicate CyphalCAN is used
#define CYPHALCAN_NUM   2

//!@brief if this is true it will output the CS wakeup
//#define OUTPUT_CS_WAKEUP
...
static int gCanModeOFFDroneCANCyphalCAN = CAN_OFF_NUM; // 0 = off, 1 = DRONECAN_NUM, 2 = CYPHALCAN_NUM
...
int bms_main(int argc, char *argv[])
{
    ...
    if(!gBmsInitialized)
    {
        ...
        cli_printf("12V on in NORMAL; only off in self-discharge\n");
...
#warning Add that CAN(0) f enabled, waiting dynamic node id alloc (semi) solid on, then if ok blink (blink different for the both CAN modes ) with timer modules
#ifndef DONT_DO_CAN
        {
...
#   if BOARD_NAME == MR_BMS771_BOARD // Only the MR-BMS771 has CAN LEDs
            // write the CAN0 and CAN1 LED pin OFF
            retValue = gpio_writePin(LED_CAN0, CAN_LED_OFF);

            // check if it went wrong
            if(retValue)
            {
                cli_printfError("GPIO ERROR: writing LED_CAN0 CAN_LED_OFF went wrong!\n");
                cli_printf("SELF-TEST GPIO: \e[31mFAIL\e[39m\n");
            }

            retValue = gpio_writePin(LED_CAN1, CAN_LED_OFF);

            // check if it went wrong
            if(retValue)
            {
                cli_printfError("GPIO ERROR: writing LED_CAN1 CAN_LED_OFF went wrong!\n");
                cli_printf("SELF-TEST GPIO: \e[31mFAIL\e[39m\n");
            }
#   endif
...
#   if BOARD_NAME == MR_BMS771_BOARD
        // initialze the SE050 or SE051
        retValue = se05x_initialize(resetCauseExWatchdog);
        if(retValue)
        {
            // output to the user
            cli_printfError("main ERROR: failed to check SE05X! code %d\n", retValue);

            // Check if the reset cause is not the watchdog
            if(!resetCauseExWatchdog)
            {
                cli_printf("SELF-TEST SE05X: \e[31mFAIL\e[39m\n");
            }
        }

        // check the tja1463 if no WD reset
        // put the TJA1463 in a lower power mode
        retValue = tja1463_initialize(resetCauseExWatchdog);
        if (retValue)
        {
            // output to the user
            cli_printfError("main ERROR: failed to check TJA1463! code %d\n", retValue);

            // Check if the reset cause is not the watchdog
            if(!resetCauseExWatchdog)
            {
                cli_printf("SELF-TEST TJA1463: \e[31mFAIL\e[39m\n");
            }
        }
#   endif
#endif
...
static int mainTaskFunc(int argc, char *argv[])
{
    ...
    retValue = gpio_registerISR((uint32_t)((1 << BCC_FAULT)), gpioIsrFunction);
...
#ifdef OUTPUT_CS_WAKEUP
    // check for CSB wakeup
    if(BMSFault & BMS_CSB_WAKEUP)
    {
        cli_printf("MCU CS wakeup of BCC detected!\n");
    }
#endif
        ...
        case NORMAL:
        {
            ...
            if(mainState != *pOldState)
            {
                ...
                *pOldState = mainState;
                ...
                // turn on the 12V_external
                cli_printf("Turning on 12V external (in NORMAL state)!\n");

#if BOARD_NAME == MR_BMS771_BOARD

                if(batManagement_set12VExtOn(true))
                {
                    // if the writing went wrong, check the pin state
                    // Maybe after some time, it is on now?
                    if(gpio_readPin(EXT_12V_ON) == false)
                    {
                        cli_printfError("main ERROR: Could not turn on 12V!\n");
                    }
                }
#endif
        ...
        case CHARGE:
        {
            ...
            if(mainState != *pOldState)
            {
                ...
                *pOldState = mainState;
                ...
                // // turn off the 12V_external
                // if(batManagement_set12VExtOn(false))
                // {
                //     cli_printfError("main ERROR: Could not turn off 12V!\n");
                // }
 ...
 * @param   currentA Address of the just measured battery current in A.
 *          If a NULL pointer is given, it will reset the timers and variables.
 * @note    It will use the CURRENT_ROLLING_AVG_SIZE time rolling average of
 *          the current (every ~3ms).
 *
 * @return  0 if succesfull, otherwise it will indicate the error
 */
int checkForTransitionCurrent(float *currentA)
{
    ...
    // setTransitionVariable(DISCHAR_VAR, true) here since the mutex is locked
    ...
    {
        // get the sleep currents
        if(data_getParameter(I_SLEEP_OC_DISCHARGE, &(variable1.uint8Var), NULL) == NULL)
        {
            cli_printfError("checkForTransitionCurrent ERROR: getting i-sleep-oc-discharge went wrong!\n");

            // set it with default, just in case
            variable1.uint8Var = I_SLEEP_OC_DISCHARGE_DEFAULT;

            // set the return variable to -1
            retValue = -1;
        }
        // get the sleep current
        if(data_getParameter(I_SLEEP_OC_CHARGE, &(variable2.uint8Var), NULL) == NULL)
        {
            cli_printfError("checkForTransitionCurrent ERROR: getting i-sleep-oc-charge went wrong!\n");

            // set it with default, just in case
            variable2.uint8Var = I_SLEEP_OC_CHARGE_DEFAULT;

            // set the return variable to -1
            retValue = -1;
        }

        // check for a discharge (discharging more than the sleep current TH)
        ...
        // check for a charge
        else if((((*currentA) * 1000)) >= variable2.uint8Var)
        {
            // reset the timeoutstarted variable
            timeOutTimeStarted = false;
        ...
        // check for a sleep current if i-batt < i-sleep-oc-charge && i-batt > i-sleep-oc-discharge
        else
        {
            // reset the charge variables
            chargeTimeStarted = false;
...
void gpioIsrFunction(int signo, siginfo_t *siginfo, void *context)
{
    ...
    switch(pinNumber)
    {
        ...
        case BCC_FAULT:
            ...
            // check if the variable is not already high
            if(!gBCCRisingEdge)
            {
...
static int setNGetEnableCanMessages(bool setNGet, bool enable)
{
    ...
    if(setNGet)
    {
        // if it is going to be turned off
        if(enableCanMessages && !enable)
        {
            // check for droneCAN
            if(gCanModeOFFDroneCANCyphalCAN == DRONECAN_NUM)
            {
                // turn OFF the CAN LED
                retValue = dronecan_setLed(false);
                if(retValue)
                {
                    cli_printfError("main ERROR: Failed to turn off CAN LED: %d\r\n", retValue);
                }
            }
            // CyphalCAN
            else if(gCanModeOFFDroneCANCyphalCAN == CYPHALCAN_NUM)
            {
                // turn OFF the CAN LED
                retValue = cyphalcan_setLed(false);
                if(retValue)
                {
                    cli_printfError("main ERROR: Failed to turn off CAN LED: %d\r\n", retValue);
                }
        ...
        else if(!enableCanMessages && enable)
        {
            // check for droneCAN
            if(gCanModeOFFDroneCANCyphalCAN == DRONECAN_NUM)
            {
                // turn ON the CAN LED
                retValue = dronecan_setLed(true);
                if(retValue)
                {
                    cli_printfError("main ERROR: Failed to turn ON CAN LED: %d\r\n", retValue);
                }
            }
            // CyphalCAN
            else if(gCanModeOFFDroneCANCyphalCAN == CYPHALCAN_NUM)
            {
                // turn ON the CAN LED
                retValue = cyphalcan_setLed(true);
                if(retValue)
                {
                    cli_printfError("main ERROR: Failed to turn ON CAN LED: %d\r\n", retValue);
                }
