 ...
 * @param   bccIndex index of the BCC cell (not 2 or 3 with a 3 cell battery): cells: 1, 2, 3, 4, 5 ... -> 0, 1, .. MAX -2, MAX -1
    ...
    float dischargeVoltage)
{
    ...
    // get the cell margin in mv
    if(data_getParameter(R_BAL, &Rbal, NULL) == NULL)
    {
        cli_printfError("Balancing ERROR: getting balance resistance went wrong!\n");
        Rbal = R_BAL_DEFAULT;
        // return error
        returnValue |= -1;
    }
    ...
    for(i = 0; i < pCommonBatteryVariables->N_cells; i++)
    {
        ...
        if(i >= FIRST_BCC_CELLS)
        {
            ...
            bccCellIndex = (N_CELLS_MAX-pCommonBatteryVariables->N_cells) + i;
        }
        else
        {
    ...
    float balanceVoltage)
{
    ...
    if(gBalanceCellEnabled)
    {
        ...
        for(i = 0; i < N_CELLS_MAX; i++)
        {
            ...
            if(gBalanceCellEnabled & (1<<i))
            {
                // map the cells (1, 2, 3, ...) to the BCC cells (1, 2, ..., N_CELLS_MAX)
                if(i >= FIRST_BCC_CELLS)
                {
                    // calculate the BCC pin index
                    bccCellIndex = (N_CELLS_MAX-pCommonBatteryVariables->N_cells) + i;
    ...
    uint8_t bccIndex, bool *done)
{
    ...
    if(bccIndex > (N_CELLS_MAX-1))
    {
        cli_printfError("checkBalancingCellDone ERROR: bccIndex > %d!\n", (N_CELLS_MAX-1));
